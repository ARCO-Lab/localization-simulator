...
class NLS:
    ...
    def eqMask3d(self, est_pose, mask):
        """ 
        Calculates the Euclidean distances from anchor points to given pose.
        ...
        Returns:
            (numpy.ndarray[float]):  Euclidean distances from the estimated pose to each anchor
        """
        est_x, est_y, est_z = est_pose
        est_distances = np.sqrt((self.anchors[:, 0] - est_x)**2 + (self.anchors[:, 1] - est_y)**2 + (self.anchors[:, 2] - est_z)**2)
        est_distances = est_distances[mask]
        return est_distances
    ...
    def estimatePose3d(self, est_pose, measurements, pose, variance, isotropic):
        """Estimate the pose using Newtons method. 
        ...
        g = grad(lambda p: (np.dot(np.transpose(pose-p),np.linalg.inv(isotropic))@(pose-p)) + np.sum((1 / (variance)) * (self.eqMask3d(p, mask) - valid_measurements) ** 2))(est_pose)
        h = hessian(lambda p: (np.dot(np.transpose(pose-p),np.linalg.inv(isotropic))@(pose-p)) + np.sum((1 / (variance)) * (self.eqMask3d(p,mask) - valid_measurements) ** 2))(est_pose)
        ...
        return np.array([est_x, est_y, est_z]), np.linalg.norm(g), h
    ...
    def rmse(self, pose, guess, distances, anchors, variance, isotropic,use3d=False):
        self.anchors = anchors
        ...
        while True:
            # print(Real)
            if use3d:
                guess, grad, _ = self.estimatePose3d(guess, distances, pose, variance, isotropic)
            else:
